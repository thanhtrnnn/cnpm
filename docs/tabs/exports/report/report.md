...
Click Thông tin khách hàng → đề xuất lớp ClientInfoPage, có ô tìm kiếm, nút Tìm và bảng danh sách khách hàng.
Quản lý nhập từ khóa và ấn Tìm → hệ thống tìm khách hàng của chi nhánh → cần chức năng searchClient() của đối tượng KhachHang.
Quản lý click một khách hàng → đề xuất lớp ClientHistoryPanel, hiển thị điểm tích lũy và bảng lịch sử.
...
Lớp Employee gọi hàm checkLogin(). Kết quả đăng nhập thành công.
Lớp Employee gửi kết quả lại cho lớp LoginPage.
Lớp LoginPage gọi sang lớp ClientInfoPage.
Lớp ClientInfoPage hiển thị cho Quản lý.
Quản lý nhập từ khóa (tên/SĐT) và click nút Tìm kiếm.
Lớp ClientInfoPage gọi lớp KhachHang để tìm kiếm (searchClient()).
Lớp KhachHang trả về danh sách khách hàng.
Lớp ClientInfoPage hiển thị danh sách cho Quản lý.
Quản lý click chọn một khách hàng.
Lớp ClientInfoPage gọi sang lớp ClientHistoryPanel.
Lớp ClientHistoryPanel gọi lớp HoaDon lấy lịch sử (getHistory()).
Lớp HoaDon trả về lịch sử sử dụng và điểm tích lũy.
Lớp ClientHistoryPanel hiển thị lịch sử cho Quản lý.
...
Giao diện tra cứu khách hàng (ClientInfoPage):
┌────────────────────────────────────────────────────────────┐
...
Giao diện lịch sử sử dụng (ClientHistoryPanel):
...
| LoginPage | Thuộc tính | - txtUsername : TextBox | Ô nhập tên đăng nhập. |
| LoginPage | Phương thức | + btnLoginClick() : void | Bắt sự kiện click nút đăng nhập. |
| ClientInfoPage | Thuộc tính | - txtKeyword : TextBox | Ô nhập từ khóa (tên hoặc SĐT). |
| ClientInfoPage | Thuộc tính | - btnSearch : Button | Nút tìm kiếm khách hàng. |
| ClientInfoPage | Thuộc tính | - tblClients : Table | Bảng danh sách khách hàng. |
| ClientInfoPage | Phương thức | + btnSearchClick() : void | Gửi từ khóa sang Controller tìm khách hàng. |
| ClientInfoPage | Phương thức | + displayClients(list : List<KhachHang>) : void | Render danh sách khách hàng lên bảng. |
| ClientInfoPage | Phương thức | + tblClientsClick(maKH : String) : void | Mở panel lịch sử của khách hàng được chọn. |
| ClientHistoryPanel | Thuộc tính | - lblPoint : Label | Hiển thị điểm tích lũy. |
| ClientHistoryPanel | Thuộc tính | - tblHistory : Table | Bảng lịch sử sử dụng. |
| ClientHistoryPanel | Phương thức | + displayHistory(list : List<HoaDon>) : void | Đổ lịch sử hóa đơn lên bảng tblHistory. |
...
| --- | --- | --- |
| LoginController | + checkLogin(username : String, password : String) : boolean | Kiểm tra tài khoản đăng nhập. |
| KhachHangController | + searchClient(keyword : String, maCN : String) : List<KhachHang> | Tìm khách hàng theo tên hoặc SĐT trong phạm vi chi nhánh. |
...
2. LoginPage gọi btnLoginClick(), gửi thông tin sang LoginController.checkLogin(username, password).
3. LoginController gọi thực thể Employee xác thực và trả kết quả thành công về LoginPage.
4. LoginPage điều hướng sang ClientInfoPage hiển thị màn hình tra cứu.
5. Quản lý nhập từ khóa (tên hoặc SĐT) và nhấn Tìm kiếm; ClientInfoPage gọi btnSearchClick().
6. ClientInfoPage gọi KhachHangController.searchClient(keyword, maCN).
7. KhachHangController gọi thực thể KhachHang truy vấn danh sách khách hàng của chi nhánh, trả List<KhachHang> về.
8. ClientInfoPage tự gọi displayClients(list) để hiển thị danh sách.
9. Quản lý click chọn một khách hàng; ClientInfoPage gọi tblClientsClick(maKH) và mở ClientHistoryPanel.
10. ClientHistoryPanel gọi KhachHangController.getHistory(maKH).
...
12. ClientHistoryPanel tự gọi displayHistory(list) hiển thị lịch sử và điểm tích lũy cho Quản lý.
...
| id | branch_id | Client_id | receipt_time | total_amount |
| --- | --- | --- | --- | --- |
